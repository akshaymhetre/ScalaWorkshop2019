...
//  }
//
//  def unWrappedFruitBox1(box: GBox[Fruit]) =
//    s"This box contains fruits : ${box.label} with quantity ${box.quantity} in dozens"
//
//
//  private val box: OrangeBox = OrangeBox(new Orange(), 3)
//  val box1: GBox[Fruit] = box;
//
//  unWrappedFruitBox1(box);
//
//
...
object PatternAssign extends App {
  def getHead[A](lst : List[A]):Option[A] = lst match {
    case head :: _ => Some(head)
    case _ => None
  }

  def getSecondElement[A](lst : List[A]):Option[A] = lst match {
    case _ :: second :: _ => Some(second)
    case _ => None
  }

  private val maybeInt: Option[Int] = getHead(List(1))
  println(maybeInt.map(_*2).get)
...
object PMForCollections{
  val langs: Seq[(String, String)] = Seq(
    ("Scala","Odersky"),
    ("Scala","Odersky"),
    ("Scala", "Akshay"),
    ("Clojure", "Rich"),
  ...
  val Person(name, _ , _) = alice;

  val Email1(name, domain) = "akshay@gslab.com"
